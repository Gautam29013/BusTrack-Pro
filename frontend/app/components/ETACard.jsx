...
function DeviationAlert({ deviationKm }) {
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontWeight: 700, fontSize: '12px', color: 'var(--accent-rose)' }}>Route Deviation Detected</div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
        <div>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Now Tracking</div>
          <div style={{ fontWeight: 700, fontSize: '18px', fontFamily: 'Space Grotesk, sans-serif' }}>Bus #{bus.busNumber || bus.number}</div>
          <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '2px' }}>{bus.routeName || 'City Route'}</div>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', padding: '16px', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.06)', marginBottom: '12px' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px' }}>Arrival progress</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
        {[
          ...
          <div key={stat.label} style={{ textAlign: 'center', padding: '10px 8px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: `1px solid ${isDeviated && stat.label === 'Off-route' ? 'rgba(244,63,94,0.2)' : 'rgba(255,255,255,0.05)'}` }}>
            ...
            <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>{stat.value}<span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{stat.unit}</span></div>
      ...
      {isAuthenticated && (
        <button
          ...
          style={{ width: '100%', marginTop: '12px', padding: '10px', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '8px', color: 'var(--accent-blue)', fontWeight: 600, fontSize: '13px', cursor: 'pointer', transition: 'background 0.2s' }}
      ...
      <div style={{ marginTop: '12px' }}>
        {!showSmsInput ? (
          <button
            ...
            style={{ width: '100%', padding: '10px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '8px', color: 'var(--accent-emerald)', fontWeight: 600, fontSize: '13px', cursor: 'pointer', transition: 'background 0.2s' }}
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} style={{ background: 'rgba(255,255,255,0.03)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '8px' }}>Enter phone number for ETA alert:</div>
